...
* I used explicit Terraform depends\_on and output variables to manage resource creation order robustly.
...
2. Jenkins pipeline would trigger Terraform to provision the green environment (new EC2 instances in a separate ASG and target group).
3. Terraform outputs included ALB target group ARNs and instance details.
4. Configuration and deployment:
5. After provisioning, Jenkins launched Ansible playbooks to install and configure the app on green instances.
6. Smoke tests and health checks ran automatically via Jenkins.
7. Traffic switch:
8. If tests passed, Jenkins updated the ALB listener rules to redirect traffic from blue target group to green target group.
9. DNS and routing stayed the same, ensuring zero downtime.
10. Rollback:
11. In case of failure, the pipeline automatically reverted the ALB traffic to blue instances.
12. Green instances were then terminated to prevent cost leakage.

***

### Technical Challenges

* Dependency management: We had to ensure RDS was fully available before EC2 instances started provisioning. We achieved this with explicit depends\_on and checking DB availability in deployment scripts.
